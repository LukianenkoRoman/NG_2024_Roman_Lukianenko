...
public class ShopItemsReporService
{
    private Dictionary<string, Func<ItemModel, object>> KeyValuePairs { get; set; } = new Dictionary<string, Func<ItemModel, object>>()
    {
        { "Name", item => item.Name },
    ...
    public void FillReportDataFromModel(XLTemplate template, ActivityReportConfiguration configuration, ShopReportModel model)
    {
        var worksheet = template.Workbook.Worksheets.First();
